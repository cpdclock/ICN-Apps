    ...
    jQuery('.azoom-team-members').each(function() {
        ...
        that.find('.team-member-article').click(function(e) {
            ...
            if (jQuery('.' + id).length) {
                no_slide = true;
                jQuery('.' + id).empty();
                jQuery('.' + id).append(jQuery('<div class="team-member-box-close">Ã—</div>'));
                jQuery('.' + id).append(content);
            } else {
                var box = jQuery('<div class="team-member-box ' + id + '"><div class="team-member-box-close">Ã—</div></div>');
                box.append(content);
                that.prepend(box);
...
function rockthemes_activate_hover() {
    ...
    for (var i = 0; i < hover_elems.length; i++) {
        ...
        on('touchend', that + ' .hover-mobile-back', {
            that: that
        }, function(e) {
